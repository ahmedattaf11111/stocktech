            ...
            <div class="row">
              <div class="col-lg-12 mb-2">
                <div class="form-group">
                  ...
                  <input
                    :placeholder="$t('TYPE_MAIN_HEADER')"
                    type="text"
                    class="form-control"
                    v-model="v$.main_header.$model"
                    :class="{
                      'is-invalid': v$.main_header.$error || titleArExist,
                    }"
                  />
                  <div class="invalid-feedback">
                    <div v-for="error in v$.main_header.$errors" :key="error">
                      {{ $t("MAIN_HEADER") + " " + $t(error.$validator) }}
                    </div>
                    <div v-if="!v$.main_header.$invalid && titleArExist">
                      {{ $t("MAIN_HEADER") + " " + $t("EXIST") }}
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-12 mb-2">
                <div class="form-group">
                  ...
                  <input
                    :placeholder="$t('TYPE_SECOND_HEADER')"
                    type="text"
                    class="form-control"
                    v-model="second_header"
                  />
                </div>
              </div>
              <div class="col-lg-12 mb-2">
                <div class="form-group">
                  ...
                  <input
                    :placeholder="$t('TYPE_SMALL_HEADER')"
                    type="text"
                    class="form-control"
                    v-model="small_header"
                  />
                </div>
              </div>
            </div>
          ...
          <div class="col-lg-12">
            <div class="form-container bg-white border border-radius-4 mb-30">
              <div class="create-item">
                ...
                <div class="form-group">
                  <textarea
                    :placeholder="$t('TYPE_SECOND_BODY')"
                    type="text"
                    class="form-control"
                    v-model="second_body"
                  >
                  ...
                  <div class="col-lg-4">
                    <div class="form-gorup">
                      <input
                        class="form-control"
                        v-model="point_three"
                        :placeholder="$t('TYPE_POINT_THREE')"
                      />
                    </div>
                  </div>
                </div>
        ...
        >
          <div class="information">
            ...
            <ul>
              ...
              <a
                id="a__about-body"
                @click="currentSection = 'about-body'"
                href="#about-body"
              >
                <li
                  :class="{ active: currentSection == 'about-body' }"
                  class="add-new-tax"
                >
                  {{ $t("BODY_CONTENT") }}
                </li>
              ...
              <a
                id="a__points"
                @click="currentSection = 'points'"
                href="#points"
              >
                <li
                  :class="{ active: currentSection == 'points' }"
                  class="add-new-tax"
                >
                  {{ $t("POINTS") }}
                </li>
              </a>
              <a id="a__video" @click="currentSection = 'video'" href="#video">
                <li
...
import aboutClient from "../../../http-clients/admin/about-client";
...
export default {
  ...
  setup(props, context) {
    ...
    onMounted(() => {
      aboutClient.getItem().then((res) => {
        if (!res.data) return;
        form.id = res.data.id;
        form.main_header = res.data.main_header;
        form.second_header = res.data.second_header;
        form.small_header = res.data.small_header;
        form.first_body = res.data.first_body;
        form.second_body = res.data.second_body;
        form.point_one = res.data.point_one;
        form.point_tow = res.data.point_tow;
        form.point_three = res.data.point_three;
        form.video_url = res.data.video_url;
    ...
    function setScrollTemp() {
      ...
      window.onscroll = function () {
        ...
        let image = $("#a__image").attr("href");
        let body = $("#a__about-body").attr("href");
        let points = $("#a__points").attr("href");
        let video = $("#a__video").attr("href");
        if (linkClicked) return;
        if (document.documentElement.scrollTop >= $(video).offset().top - 140) {
          data.currentSection = "video";
        } else if (
          document.documentElement.scrollTop >=
          $(points).offset().top - 140
        ) {
          data.currentSection = "points";
        } else if (
          document.documentElement.scrollTop >=
          $(body).offset().top - 140
        ) {
          data.currentSection = "about-body";
        } else if (
          document.documentElement.scrollTop >=
          $(image).offset().top - 140
        ) {
    ...
    function getForm() {
      return {
        ...
        media_manager_id: data.selectedMedia.length
          ...
          : null,
        main_header: form.main_header,
        second_header: form.second_header,
        small_header: form.small_header,
        point_one: form.point_one,
        point_tow: form.point_tow,
        point_three: form.point_three,
        first_body: form.first_body,
        second_body: form.second_body,
