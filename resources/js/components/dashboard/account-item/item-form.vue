              ...
              <div class="row">
                ...
                <div class="col-lg-12 mb-2">
                  <div class="form-group">
                    ...
                    <Multiselect
                      :placeholder="$t('DETECTED_FOR_USER')"
                      :label="$i18n.locale == 'ar' ? 'name_ar' : 'name_en'"
                      v-model="is_user"
                      valueProp="value"
                      :options="detectedUsers"
                    />
                  </div>
                </div>
                <div class="col-lg-12 mb-2">
      ...
      <div class="col-lg-3 order-lg-2 order-1">
        <div class="bg-white border border-radius-4 mb-30">
          <div class="information">
            <div class="title">{{ $t("ACCOUNT_ITEM_INFORMATION") }}</div>
            <ul>
...
import accountItemClient from "../../../http-clients/admin/account-item-client";
...
export default {
  ...
  setup(props, context) {
    ...
    const data = reactive({
      ...
      titleEnExist: false,
      detectedUsers: [
        { value: 0, name_ar: "غير محدد", name_en: "None" },
        { value: 1, name_ar: "موظف", name_en: "Employee" },
        { value: 2, name_ar: "طالب", name_en: "Student" },
      ],
      effects: [
        { value: 0, name_ar: "واردات", name_en: "Revenues" },
        { value: 1, name_ar: "مصروفات", name_en: "Expenses" },
      ],
    });
    const form = reactive({
      name: "",
    ...
    onMounted(() => {
      accountItemClient.getItem(route.params.id).then((res) => {
        form.name = res.data.name;
        form.is_user = res.data.is_user;
        form.effect = res.data.effect;
      });
    });
    //Methods
