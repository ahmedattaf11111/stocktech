      ...
      >
        ...
        >
          <div class="create-item">
            ...
            <div v-for="rate in all_rates" :key="rate.id" class="row mb-2">
              <div class="col-lg-4">
                <div class="rate-name border">
                  {{ rate.name }}
                </div>
              </div>
              <div class="col-lg-8">
                <div class="rates">
                  <div
                    class="rate"
                    @click="incrementValue(rate, index)"
                    :class="{ active: index - 1 < rate.value }"
                    v-for="index in rate.length"
                    :key="index"
                  >
      ...
      <div class="col-lg-3 order-lg-2 order-1">
        ...
        >
          <div class="information">
            <div class="title">{{ $t("GENERAL_RATE_INFORMATION") }}</div>
            <ul>
              <a
              ...
              <a id="a__rates" @click="currentSection = 'rates'" href="#rates">
                <li :class="{ active: currentSection == 'rates' }">
                  {{ $t("RATES") }}
                </li>
              </a>
            </ul>
...
import generalRateClient from "../../../http-clients/admin/general-rate-client";
...
export default {
  ...
  setup(props, context) {
    ...
    //Methods
    function incrementValue(rate, index) {
      if (index - 1 < rate.value) {
        rate.value--;
      } else {
        rate.value++;
      }
    ...
    function setForm() {
      if (route.params.id != "create") {
        generalRateClient.getItem(route.params.id).then((res) => {
          form.student_id = res.data.student_id;
          form.employee_id = res.data.employee_id;
          getRates(res.data.rates);
        });
      } else {
        getRates([]);
      }
    ...
    function setScrollTemp() {
      ...
      window.onscroll = function () {
        ...
        let addhashElement = $("#a__rates").attr("href");
        if (linkClicked) return;
        if (
          document.documentElement.scrollTop >=
          $(addhashElement).offset().top - 140
        ) {
          data.currentSection = "rates";
        } else {
    ...
    function getEmployees() {
      generalRateClient.getEmployees().then((res) => {
        data.employees = res.data;
      });
    }
    function getRates(storedRates) {
      generalRateClient.getRates().then((res) => {
        data.all_rates = res.data.map((r) => {
          let result = storedRates.filter((rat) => {
            return rat.id == r.id;
          });
          r.value = result.length ? result[0].value : 0;
          return r;
        });
      });
    }
    function getStudents() {
      generalRateClient.getStudents(form.employee_id).then((res) => {
