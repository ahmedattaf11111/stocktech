          ...
          <div class="create-item">
            ...
            <form>
              <div class="row">
                ...
                <div class="col-lg-12 mb-2">
                  <div class="form-group">
                    <label>{{ $t("RATE_LENGTH") }}</label>
                    <input
                      :placeholder="$t('TYPE_RATE_LENGTH')"
                      type="number"
                      class="form-control"
                      v-model="v$.length.$model"
                      :class="{
                        'is-invalid': v$.length.$error ,
                      }"
                    />
                    <div class="invalid-feedback">
                      <div v-for="error in v$.length.$errors" :key="error">
                        {{ $t("RATE_LENGTH") + " " + $t(error.$validator) }}
                      </div>
                    </div>
                  </div>
                </div>
...
import rateClient from "../../../http-clients/admin/rate-client";
...
export default {
  ...
  setup(props, context) {
    ...
    onMounted(() => {
      rateClient.getItem(route.params.id).then((res) => {
        form.name = res.data.name;
        form.length = res.data.length;
      });
    });
    //Methods
