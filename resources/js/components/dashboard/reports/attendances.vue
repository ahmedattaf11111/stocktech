<template>
  <div class="total-hour-report-container">
    ...
    <div class="page-header border" style="margin-top: 10px">
      <div class="row">
        <div class="col-md-6 col-sm-12">
          <div class="title">
            {{ $t("Attendances and Absences Statement") }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="table-container">
        <div class="controls">
          <div class="row">
            ...
            <div class="sessions" style="padding: 0 8px; width: 20%">
              <Multiselect
          ...
          <table class="table" id="printMe">
            ...
            <tbody v-if="items.length">
              <tr v-for="(item, index) in items" :key="item.id">
                ...
                <td>
                  {{ item.attendances + item.absences }}
                </td>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      reportClient.getEmployees().then((res) => {
        data.employees = res.data;
      });
      reportClient.getStudents().then((res) => {
    ...
    function getAttendancesReport() {
      let item_id = data.type == 1 ? data.employee_id : data.student_id;
      reportClient
        .getAttendancesReport(data.start, data.end, item_id, data.type)
        .then((res) => (data.items = [res.data]));
    }
    function searchItems() {
      let item_id = data.type == 1 ? data.employee_id : data.student_id;
      if (!data.start || !data.end || !item_id) return;
