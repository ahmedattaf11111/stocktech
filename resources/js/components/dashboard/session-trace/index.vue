...
import sessionTraceClient from "../../../http-clients/admin/session-trace-client";
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      getItems();
      sessionTraceClient.getEmployees().then((res) => {
        data.employees = res.data;
      });
      sessionTraceClient.getAllSessions().then((res) => {
        data.all_sessions = res.data.map((session) => {
          session.name = parseTimeDate(session);
          return session;
        });
      });
      sessionTraceClient.getAllStudents().then((res) => {
        data.all_students = res.data;
      });
    });
    //Methods
    ...
    function storeSessionTrace() {
      sessionTraceClient
        .store({
          employee_id: form.employee_id,
          student_id: form.student_id,
          session_id: form.session_id,
          pages_no: form.pages_no,
        })
        .then((res) => {
          getItems();
    ...
    function getSessions(id) {
      if (!id) return;
      sessionTraceClient.getSessions(id).then((res) => {
    ...
    function downloadExcelFile() {
      sessionTraceClient.getAllItems().then((res) => {
        let data = res.data;
    ...
    function onEmployeeSelected(id) {
      setTimeout(() => {
        sessionTraceClient.getSessions(form.employee_id).then((res) => {
          data.sessions = res.data.map((session) => {
            session.name = parseTimeDate(session);
            return session;
          });
        });
        sessionTraceClient.getStudents(form.employee_id).then((res) => {
