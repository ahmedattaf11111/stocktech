          ...
          <form @submit.prevent="">
            <div class="modal-body">
              ...
              <div class="form-group">
                <Multiselect
                  @change="onAccountItemSelected"
                  :placeholder="$t('SELECT_ACCOUNT_ITEM')"
                  v-model="account_item_id"
                  valueProp="id"
                  :label="'name'"
                  :options="account_items"
                />
              </div>

              <div
                v-if="currentAccountItem && currentAccountItem.is_user == 1"
                class="form-group"
              >
                <Multiselect
              ...
              <div
                v-if="currentAccountItem && currentAccountItem.is_user == 2"
                class="form-group"
              >
                <Multiselect
              ...
              <div v-if="currentAccountItem" class="form-group">
                <input
                  class="form-control"
                  v-model="amount"
                  @blur="onAmountBlur"
          ...
          <div class="row">
            ...
            <div class="sessions" style="padding: 0 8px; width: 16%">
              <Multiselect
                ...
                :label="'name'"
                :searchable="true"
                v-model="selected_account_item_id"
                valueProp="id"
                :options="account_items"
              />
            </div>
...
import transactionClient from "../../../http-clients/admin/transaction-client";
...
export default {
  ...
  setup() {
    ...
    created();
    onMounted(() => {
      transactionClient.getEmployees().then((res) => {
        data.employees = res.data;
      });
      transactionClient.getAccountItems().then((res) => {
        data.account_items = res.data;
      });

      transactionClient.getStudents().then((res) => {
    ...
    function onAmountBlur() {
      let factor = 1;
      factor = data.currentAccountItem.effect == 0 ? 1 : -1;
      form.amount = factor * Math.abs(form.amount);
    }
    function onAccountItemSelected(id) {
      let res = data.account_items.filter((a) => {
        return a.id == id;
      });
      data.currentAccountItem = res.length ? res[0] : null;
    }

    function disableForm() {
      if (data.currentAccountItem) {
        if (data.currentAccountItem.is_user == 1) {
          return !form.employee_id;
        } else if (data.currentAccountItem.is_user == 2) {
    ...
    function getSessions(id) {
      if (!id) return;
      transactionClient.getSessions(id).then((res) => {
    ...
    function downloadExcelFile() {
      transactionClient.getAllItems().then((res) => {
        let data = res.data;
