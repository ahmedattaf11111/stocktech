		...
		methods: {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/blog-categories/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/blog-categories/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
				...
				fv.validate().then((status) => {
					if (status != "Invalid") {
						...
						if (!this.id) {
							...
							this.loading = true;
							axios.post(`${baseUrl}/admin/blog-categories/${this.id}`, formData).then(res => {
								this.loading = false;
								$(".btn-reset").click();
