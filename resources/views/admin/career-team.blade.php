...
<div id="app" class="pd-ltr-20 xs-pd-20-10">
			...
			<div class="modal-content p-3 p-md-5">
				<div class="modal-body">
					...
					<div class="text-center mb-4">
						<h3 class="mb-2">{{__('general.New Career Team')}}</h3>
						<p class="text-muted">{{__('general.Add a new Career Team and attach it to a career')}}</p>
					</div>
					<form id="form-add-new-record" class="row g-3" @submit.prevent="save">
						...
						<div class="col-12 text-center">
							@if(auth()->user()->can('super admin')||auth()->user()->can('update career-team'))
	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="py-3 mb-4"><span class="text-muted fw-light">{{__('general.Website')}} /</span> {{__('general.Career Team List')}}</h4>
			<!-- DataTable with Buttons -->
			<div class="card">
				<div class="card-datatable table-responsive pt-0">
					<div class="data-table-header">
						<div class="actions">
							...
							@if(auth()->user()->can('super admin')||auth()->user()->can('create career-team'))
							<button @click="onAddClicked" class="create-new btn waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#form-add-new-record">
								<i class="ti ti-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">{{__('general.Add Career Team')}}</span>
							</button>
							@endif
						</div>
						<div class="search-cntrl">
							@if(auth()->user()->can('super admin')||auth()->user()->can('delete career-team'))
							...
							@endif
							<input @keyup.enter="search" v-model="text" placeholder="{{__('general.Search career team')}}" class="form-control" />
						</div>
					</div>
						...
						<template v-if="!loading&&items.data.length">
							<tbody>
								<tr v-for="item in items.data">
									...
									<td>#@{{item.id}}</td>
									<td>@{{translate("name", item.name, "CareerTeam", item.id)}}</td>
									<td>
										<div class="d-flex align-items-center">
											<a data-bs-target="#form-add-new-record" data-bs-toggle='modal' @click="show(item)" href="javascript:;" class="text-body">
												<i class="ti ti-{{auth()->user()->can('super admin')||auth()->user()->can('update career-team')?'edit':'eye'}} ti-sm me-2"></i>
											</a>
											@if(auth()->user()->can('super admin')||auth()->user()->can('delete career-team'))
		...
		methods: {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/career-team/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/career-team/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
			...
			show(item) {
				fv.resetForm();
				this.id = item.id;
				this.name = this.translate("name", item.name, "CareerTeam", this.id);
				uploadedFiles = [];
			},
			onAddClicked() {
				fv.resetForm();
			...
			save() {
				fv.validate().then((status) => {
					if (status != "Invalid") {
						...
						if (!this.id) {
							this.loading = true;
							axios.post(`${baseUrl}/admin/career-team`, formData).then(res => {
								this.loading = false;
								this.name = "";
								$(".btn-reset").click();
								this.page = 1;
							...
							this.loading = true;
							axios.post(`${baseUrl}/admin/career-team/${this.id}`, formData).then(res => {
								this.loading = false;
								$(".btn-reset").click();
