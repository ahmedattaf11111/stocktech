...
<div id="app" class="pd-ltr-20 xs-pd-20-10">
	...
	<div class="bs-toast toast requirment-toast-ex toast-ex animate__bounce animate__animated my-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="2000">
		<div class="toast-header">
			<i class="ti ti-bell ti-xs me-2"></i>
		...
		<div class="toast-body">{{__('general.Requirment is required')}}</div>
	</div>

	<div class="bs-toast toast type-toast-ex toast-ex animate__bounce animate__animated my-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="2000">
		<div class="toast-header">
			<i class="ti ti-bell ti-xs me-2"></i>
			<div class="me-auto fw-medium">{{__('general.Error')}}</div>
			<small class="text-muted">{{__('general.one seconds ago')}}</small>
		...
		<div class="toast-body">{{__('general.Type is required')}}</div>
	</div>
	<div class="bs-toast toast team-toast-ex toast-ex animate__bounce animate__animated my-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="2000">
		<div class="toast-header">
			<i class="ti ti-bell ti-xs me-2"></i>
			<div class="me-auto fw-medium">{{__('general.Error')}}</div>
			<small class="text-muted">{{__('general.one seconds ago')}}</small>
		...
		<div class="toast-body">{{__('general.Career team is required')}}</div>
	</div>
	<div class="bs-toast toast location-toast-ex toast-ex animate__bounce animate__animated my-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="2000">
		<div class="toast-header">
			<i class="ti ti-bell ti-xs me-2"></i>
			<div class="me-auto fw-medium">{{__('general.Error')}}</div>
			<small class="text-muted">{{__('general.one seconds ago')}}</small>
		...
		<div class="toast-body">{{__('general.Location is required')}}</div>
	</div>
	...
	<div class="offcanvas offcanvas-end" id="add-new-record">
		<div class="offcanvas-header border-bottom">
			<h5 class="offcanvas-title" id="exampleModalLabel">{{__('general.New Career')}}</h5>
		...
		<div class="row offcanvas-body flex-grow-1">
			...
			<form style="padding-right: 0 !important;padding-left: 0 !important;" class="add-new-record" id="form-add-new-record" @submit.prevent="save">
				<div class="card-cont">
					...
					<div class="col-sm-12">
						<div class="mb-2">
							<label for="categorySelect2Primary" class="form-label">{{__('general.Career Team')}}</label>
					...
					<div class="col-sm-12">
						<div class="mb-2">
							<label for="categorySelect3Primary" class="form-label">{{__('general.Location')}}</label>
							<div class="select2-primary">
								<select id="categorySelect3Primary" name="location_id" class="select2 form-select">
									<option :selected="location_id==location.id" v-for="location in locations" :value="location.id">
										@{{location.name}}
									</option>
								</select>
							</div>
					...
					<div class="col-sm-12">
						<div class="mb-2">
							<label for="categorySelect4Primary" class="form-label">{{__('general.Type')}}</label>
							<div class="select2-primary">
				...
				<div class="card-cont mt-3">
					...
					<div class="col-sm-12">
						<div class="mb-2">
							...
							<textarea name="description" class="form-control" id="description" v-model="description" rows="3" placeholder="{{__('general.description')}}"></textarea>
						</div>
					</div>
					<div class="col-sm-12">
						<h5 class="card-header">
							<label class="form-label" for="basicPost">{{__('general.requirments')}}</label>
						</h5>
						<div class="card-body">
							<div id="full-editor">
							</div>
			...
			<div class="col-sm-12 mt-4 mb-2" style="padding-right: 0;padding-left:0">
				@if(auth()->user()->can('super admin')||auth()->user()->can('update career'))
	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="py-3 mb-4"><span class="text-muted fw-light">{{__('general.Website')}} /</span> {{__('general.Career List')}}</h4>
			<!-- DataTable with Buttons -->
			<div class="card">
				<div class="card-datatable table-responsive pt-0">
					<div class="data-table-header">
						<div class="actions">
							...
							@if(auth()->user()->can('super admin')||auth()->user()->can('create career'))
							<button @click="onAddClicked" data-bs-target="#add-new-record" data-bs-toggle='offcanvas' class="create-new btn  waves-effect waves-light">
								<i class="ti ti-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">{{__('general.Add career')}}</span>
							</button>
							@endif
						</div>
						<div class="search-cntrl">
							...
							@endif
							<input @keyup.enter="search" v-model="text" placeholder="{{__('general.Search career')}}" class="form-control" />
						</div>
					</div>
	...
	new Vue({
		...
		mounted() {
			$('#categorySelect2Primary').on('change', () => {
				...
				this.career_team_id = ids[0];
			});
			$('#categorySelect3Primary').on('change', () => {
				var data = $("#categorySelect3Primary option:selected");
				let ids = [];
				for (const [key, value] of Object.entries(data)) {
					if (Number.isInteger(parseInt(key))) {
						ids.push(data[key].getAttribute("value"));
				...
				this.location_id = ids[0];
			});
			$('#categorySelect4Primary').on('change', () => {
				var data = $("#categorySelect4Primary option:selected");
				let ids = [];
				for (const [key, value] of Object.entries(data)) {
					if (Number.isInteger(parseInt(key))) {
						ids.push(data[key].getAttribute("value"));
		...
		methods: {
			...
			deleteImage(image) {
				this.loading = true;
				axios.delete(`${baseUrl}/admin/careers/delete-image/${this.id}/${image}`).then(res => {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/careers/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/careers/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
			...
			onAddClicked() {
				...
				this.career_team_id = this.teams.length ? this.teams[0].id : '';
				this.career_type_id = this.types.length ? this.types[0].id : '';
				this.location_id = this.locations.length ? this.locations[0].id : '';
				this.images = [];
				uploadedFiles = [];
			},
			save() {
				fv.validate().then((status) => {
					if (status != "Invalid") {
						...
						formData.append("career_type_id", this.career_type_id);
						formData.append("career_team_id", this.career_team_id);
						formData.append("location_id", this.location_id);
						formData.append("description", this.description);
						formData.append("requirment", fullEditor.root.innerHTML);
						uploadedFiles.forEach((file, index) => {
							formData.append(`images[${index}]`, file);
						})
						if (!this.career_type_id) {
							const toastAnimationExample = document.querySelector(`.type-toast-ex`);
						...
						if (!this.career_team_id) {
							const toastAnimationExample = document.querySelector(`.team-toast-ex`);
							toastAnimationExample.querySelector('.ti').classList.add("text-danger");
							toastAnimation = new bootstrap.Toast(toastAnimationExample);
							toastAnimation.show();
							return;
						...
						if (!this.location_id) {
							const toastAnimationExample = document.querySelector(`.location-toast-ex`);
							toastAnimationExample.querySelector('.ti').classList.add("text-danger");
							toastAnimation = new bootstrap.Toast(toastAnimationExample);
							toastAnimation.show();
							return;
						...
						if (fullEditor.root.innerHTML == "<p><br></p>") {
							const toastAnimationExample = document.querySelector(`.requirment-toast-ex`);
							toastAnimationExample.querySelector('.ti').classList.add("text-danger");
							toastAnimation = new bootstrap.Toast(toastAnimationExample);
							toastAnimation.show();
							return;
						...
						if (!this.id) {
							...
							this.loading = true;
							axios.post(`${baseUrl}/admin/careers`, formData).then(res => {
								this.loading = false;
								$("#cancel-modal").click();
								this.page = 1;
							...
							this.loading = true;
							axios.post(`${baseUrl}/admin/careers/${this.id}`, formData).then(res => {
								this.loading = false;
								$("#cancel-modal").click();
								this.page = 1;
