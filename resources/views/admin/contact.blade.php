...
<div id="app" class="pd-ltr-20 xs-pd-20-10">
			...
			<div class="modal-content p-3 p-md-5">
				<div class="modal-body">
					...
					<div class="text-center mb-4">
						<h3 class="mb-2">{{__('general.Contact Message')}}</h3>
	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="py-3 mb-4"><span class="text-muted fw-light">{{__('general.Website')}} /</span> {{__('general.Contact List')}}</h4>
			<!-- DataTable with Buttons -->
			<div class="card">
				<div class="card-datatable table-responsive pt-0">
					<div class="data-table-header">
						...
						<div class="search-cntrl">
							@if(auth()->user()->can('super admin')||auth()->user()->can('delete contact'))
							...
							@endif
							<input @keyup.enter="search" v-model="text" placeholder="{{__('general.Search contact')}}" class="form-control" />
						</div>
					</div>
								...
								<tr v-for="item in items.data">
									...
									<td>
										<div class="d-flex align-items-center">
											<a data-bs-toggle="modal" data-bs-target="#form-add-new-record" @click="show(item)" href="javascript:;" class="text-body"><i class="ti ti-eye ti-sm me-2"></i></a>
											@if(auth()->user()->can('super admin')||auth()->user()->can('delete contact'))
		...
		methods: {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/contact/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/contact/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
