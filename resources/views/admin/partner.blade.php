...
<div id="app" class="pd-ltr-20 xs-pd-20-10">
	...
	<div class="offcanvas offcanvas-end" id="add-new-record">
		<div class="offcanvas-header border-bottom">
			<h5 class="offcanvas-title" id="exampleModalLabel">{{__('general.New Partner')}}</h5>
		...
		<div class="row offcanvas-body flex-grow-1">
			...
			<form style="display:none;padding-right: 0 !important;padding-left: 0 !important;" class="add-new-record" id="form-add-new-record" @submit.prevent="save">
			...
			<div class="col-sm-12 mt-4 mb-2" style="padding-right: 0;padding-left:0">
				@if(auth()->user()->can('super admin')||auth()->user()->can('update partner'))
	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="py-3 mb-4"><span class="text-muted fw-light">{{__('general.Website')}} /</span> {{__('general.Partner List')}}</h4>
			<!-- DataTable with Buttons -->
			<div class="card">
				<div class="card-datatable table-responsive pt-0">
					<div class="data-table-header">
						<div class="actions">
							...
							@if(auth()->user()->can('super admin')||auth()->user()->can('create partner'))
							<button @click="onAddClicked" data-bs-target="#add-new-record" data-bs-toggle='offcanvas' class="create-new btn  waves-effect waves-light">
								<i class="ti ti-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">{{__('general.Add partner')}}</span>
							</button>
							@endif
						</div>

						<div class="search-cntrl">
							@if(auth()->user()->can('super admin')||auth()->user()->can('delete partner'))
							...
							@endif
							<input @keyup.enter="search" v-model="text" placeholder="{{__('general.Search partner')}}" class="form-control" />
						</div>
					</div>
									...
									<td>
										<div class="d-flex align-items-center">
											<a data-bs-target="#add-new-record" data-bs-toggle='offcanvas' @click="show(item)" href="javascript:;" class="text-body">
												<i class="ti ti-{{auth()->user()->can('super admin')||auth()->user()->can('update partner')?'edit':'eye'}} ti-sm me-2"></i>
											</a>
											@if(auth()->user()->can('super admin')||auth()->user()->can('delete partner'))
		...
		methods: {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/partners/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/partners/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
			...
			save() {
				...
				if (!this.id) {
					...
					this.loading = true;
					axios.post(`${baseUrl}/admin/partners`, formData).then(res => {
						this.loading = false;
						this.name = "";
						this.job = "";
						$("#cancel-modal").click();
					...
					this.loading = true;
					axios.post(`${baseUrl}/admin/partners/${this.id}`, formData).then(res => {
						this.loading = false;
						$("#cancel-modal").click();
						this.page = 1;
