...
<div id="app" class="pd-ltr-20 xs-pd-20-10">
			...
			<div class="modal-content p-3 p-md-5">
				<div class="modal-body">
					...
					<div class="text-center mb-4">
						<h3 class="mb-2">{{__('general.New Category')}}</h3>
						<p class="text-muted">{{__('general.Add a new category and attach it to a blog')}}</p>
					</div>
	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="py-3 mb-4"><span class="text-muted fw-light">{{__('general.Website')}} /</span> {{__('general.Category List')}}</h4>
			<!-- DataTable with Buttons -->
			<div class="card">
				<div class="card-datatable table-responsive pt-0">
					<div class="data-table-header">
						<div class="actions">
							...
							@if(auth()->user()->can('super admin')||auth()->user()->can('create project-category'))
						...
						<div class="search-cntrl">
							...
							@endif
							<input @keyup.enter="search" v-model="text" placeholder="{{__('general.Search category')}}" class="form-control" />
						</div>
					</div>
		...
		methods: {
			...
			onDeleteClicked(ids) {
				Swal.fire({
					...
					if (result.value) {
						axios.post(`${baseUrl}/admin/project-categories/multi-delete`, {
							ids: ids
						}).then(res => {
			...
			getItems(action = "") {
				this.loading = true;
				axios.get(`${baseUrl}/admin/project-categories/data?page=${this.page}&page_size=${this.page_size}&text=${this.text}`).then(res => {
					this.items = res.data;
					this.loading = false;
					if (action) {
			...
			save() {
				fv.validate().then((status) => {
					if (status != "Invalid") {
						...
						if (!this.id) {
							this.loading = true;
							axios.post(`${baseUrl}/admin/project-categories`, formData).then(res => {
								this.loading = false;
								this.name = "";
								$(".btn-reset").click();
								this.page = 1;
							...
							this.loading = true;
							axios.post(`${baseUrl}/admin/project-categories/${this.id}`, formData).then(res => {
								this.loading = false;
								$(".btn-reset").click();
